...
[Matching native 1.2.11 Windows and Linux packages](https://github.com/MiniLangProject/MiniLangCompilerML/releases/tag/v1.2.11)
...
`MiniLang Compiler 1.2.11`. `python mlc_win64.py --help` prints the full option
list.
...
junctions, matching `Path.rglob()` and preventing cyclic traversal. A quoted
import that explicitly names a linked file is still followed and fingerprinted.
...
Current audited Windows fixed point (2026-09-13): this Python compiler produced
...
Generated HTML and Markdown are written below `docs/api/std`. The checked-in
configuration disables generation timestamps, so unchanged standard-library
sources and revisions produce byte-identical trees on independent runs.
